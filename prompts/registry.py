...
class PromptDefinition:
    ...
    def build(self, overrides: Optional[Dict[str, str]] = None, params: Optional[Dict[str, Any]] = None) -> str:
        """
        从插槽内容组装完整 system prompt。
        overrides: 用户覆盖的 slot 内容 (key → text)
        params: 动态参数 (如 max_duration)
        """
...
def slot(key: str, default_content: str, editable: bool = True) -> PromptSlot:
    return PromptSlot(key=key, default_content=default_content, editable=editable)


def resolve(overrides: Dict[str, str], slots: List[PromptSlot], key: str) -> str:
    """解析单个 slot 的值: overrides > default"""
    if key in overrides:
...
# ═══════════════════════════════════════════════════════════════════
# 1. script_parse — 剧本结构化解析
# ═══════════════════════════════════════════════════════════════════

SCRIPT_PARSE_ROLE = """你是一位资深剧本监制和结构化编辑，擅长将叙事文本**解析**为适合动画短片流水线的结构化剧本 JSON。
...
SCRIPT_PARSE_FIDELITY = """=== 原文保真度（最高优先级——此规则优先于所有其他规则）===
...
SCRIPT_PARSE_FORMAT = """输出单个JSON对象：
{
  "title": "引人入胜的标题",
...
SCRIPT_PARSE_RULES = """故事编辑原则（**在原文保真度的前提下**应用，任何与保真度冲突的条款都以保真度优先）：
...
SCRIPT_PARSE_LANGUAGE = """【关键语言规则】JSON中的所有文本内容（title、synopsis、setting、description、mood、对白text、emotion）必须使用与原文相同的语言。中文原文 → 中文输出。不要翻译成英文。
...
class ScriptParsePrompt(PromptDefinition):
    def __init__(self):
        super().__init__(
            key="script_parse",
            category="script",
            description="剧本结构化解析 — 原文→JSON",
        )
        self.slots = [
            slot("role", SCRIPT_PARSE_ROLE, editable=True),
            slot("fidelity", SCRIPT_PARSE_FIDELITY, editable=True),
            slot("format", SCRIPT_PARSE_FORMAT, editable=False),
            slot("rules", SCRIPT_PARSE_RULES, editable=True),
            slot("language", SCRIPT_PARSE_LANGUAGE, editable=False),
        ]
