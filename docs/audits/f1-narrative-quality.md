...
**审计文件**: `core/llm_client.py`, `core/prompt_runner.py`, `prompts/registry.py`, `prompts/_base.py`, `prompts/defaults/{script_parse,character_extract,shot_split,script_generate,script_split}.py`, `scripts/gen_prompts.py`, `scripts/gen_s4_shots.py`
...
| 🟢 低 | `scripts/gen_prompts.py` | 硬编码项目路径 `projects/last_bento`，且只处理 3 个 scene 的 location hint | 无法复用于其他项目 | 改为接受命令行参数，或从 s4_shots.json 的 setting 字段自动提取 |
...
7. **S2→S3 visualAnchors 消费链验证**：S2 输出的锚点 key 与 gen_prompts.py 的消费 key 不一致，无门控拦截
...
物理约束、构图指南、转场规则、motionScript 四层交织——设计完善。扣分点：(1) 缺少程序化校验，prompt 约束依赖 LLM 自觉；(2) 手工脚本 gen_s4_shots.py 产出完全不同 schema 的数据，造成管线混乱。
...
4. **P2 (重要)**：清理 registry.py 死代码，废弃 gen_s4_shots.py 或写适配层
