...
**版本**: v1.1.0
**创建**: 2026-06-28
**更新**: 2026-07-05 (Phase 2-4: 状态管理 v2 + 并行执行 + 测试覆盖)
...
12 个阶段，前 4 个（S1/S2/S4/S4b）由 LLM/Python 完成，后 8 个（S2b/S3/S3b/S5/S6/S7/S8/S9）由 Python 脚本驱动 ComfyUI + FFmpeg + Whisper。
...
## Phase 2: 状态管理 v2 (State Manager)

**目标**: 从简单状态跟踪升级为健壮的管线执行引擎。

### 核心变更
| 特性 | v1 (旧) | v2 (新) |
|------|---------|---------|
| 数据源 | 硬编码 stage 列表 | `pipeline.yaml` 声明式加载 |
| 依赖图 | 线性顺序 | 有向无环图 (DAG) 拓扑排序 |
| Stage 数量 | 10 | 12 (新增 S2b, S4b) |
| 断点续跑 | 无 | `checkpoint` shot-level 进度恢复 |
| 参数变更 | 无 | `params_hash` SHA256 变更检测 |
| 脏传播 | 手动 | `mark_stale` BFS 自动级联 |
| 写保护 | 无 | 原子写入 (`_write_atomic`) |
| 迁移 | 无 | `migrate_v1_to_v2` 自动升级旧 state.json |

### API 一览
| 方法 | 用途 |
|------|------|
| `init_project(project)` | 创建新项目 state |
| `load(project)` | 读取并缓存 state (自动迁移 v1→v2) |
| `update_stage(project, stage, status)` | 更新阶段状态 |
| `mark_completed / mark_failed / mark_running` | 快捷状态设置 |
| `init_checkpoint(project, stage, total_shots)` | 初始化 checkpoint |
| `record_checkpoint(project, stage, shot_id)` | 记录单帧进度 |
| `get_checkpoint(project, stage)` | 读取 checkpoint |
| `skip_completed_shots(project, stage)` | 获取已完成的 shot ID 列表 |
| `record_params(project, stage, args)` | 记录参数 hash |
| `params_changed(project, stage, new_args)` | 检测参数是否变更 |
| `mark_stale(project, from_stage)` | BFS 级联标记脏 |
| `mark_dirty / clear_dirty` | 脏标记管理 |
| `next_pending(project)` | 拓扑序找下一个可执行 stage |
| `progress(project)` | 格式化进度报告 |

详见 `docs/state_manager_v2.md`。

---

## Phase 3: 并行执行 (Parallel Executor)

**目标**: 在无 GPU 竞争的 stage 之间实现安全并行，减少总管线时间。

### 设计原则
- `ThreadPoolExecutor` max_workers=2 (保守)
- **失败隔离**: 一个 stage 失败不影响其他并行 stage
- GPU 协调由调用方负责 (`ensure_comfyui` → 并行 → ...)
- 每个 stage 作为独立 subprocess 启动，state.json 写入互不干扰

### 并行化规则

| 条件 | 可并行? |
|------|---------|
| 两个 `gpu: none` stage | ✅ 是 (例: S7+S8) |
| 一个 `gpu: none` + 一个 `gpu: comfyui` | ✅ 是 (例: S4+S6) |
| 两个 `gpu: comfyui` | ❌ 否 (GPU 争用) |
| 任何 `gpu: qw35_vl` | ❌ 否 (独占模式) |
| LLM stage + script stage (同依赖集) | ✅ 是 |

### 典型并行组
| 组 | Stages | 理由 |
|----|--------|------|
| P1 | S8 (subtitles) + S9 (TTS) | 都依赖 S7，无 GPU |
| P2 | S4 (LLM) + S3 (ComfyUI) | 不同 GPU 需求，同一依赖 s2 |
