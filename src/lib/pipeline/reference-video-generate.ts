...
export async function handleReferenceVideoGenerate(task: Task) {
  ...
  // 2. Idempotency: skip if already completed OR currently generating
  const existing = await getActiveAsset(shot.id, "reference_video", 0);
  if (existing) {
    if (existing.status === "completed" && existing.fileUrl) {
      await db.update(shots).set({ status: "completed" }).where(eq(shots.id, shot.id));
      return { shotId: shot.id, referenceVideoUrl: existing.fileUrl, skipped: true };
    }
    if (existing.status === "generating") {
      // Another task is generating — wait and retry
