      ...
      "episodes": [
        {
          ...
          "t2iStructure": {
            "age": "年龄描述（如 15-year-old teenager 或 71-year-old, frail, aged）",
            "subject": "主体：性别+身高+体型关键词（英文），如 male, 162cm, thin, slightly hunched",
            "body": "体态细节：骨骼标志物、姿势、肌肉/脂肪状态（英文）",
            "face": "面部细节：脸型骨架、五官、皮肤质感、表情（英文）",
            "hair": "发型细节：颜色、长度、质地、样式（英文）",
            "clothing": "服装细节：款式+合身度+材质+颜色（英文）",
            "lighting": "光影：光位+质感+对体型的效果（英文）"
          }
        }
      ]
    }
...
⚠️ 体型必须用骨骼标志物或脂肪分布具象化——禁止只写一个形容词了事：
• 瘦（精瘦/纤细/瘦削/皮包骨）：必须描述至少3个骨骼标志物——肩胛骨凸起程度、锁骨凹陷深度、肋骨隐约可见程度、手腕/脚踝骨节突出、手指细长/指节分明。注意区分"骨架形状"和"皮肉状态"："圆脸骨架"（头骨圆润但皮肉绷紧贴骨、颧骨突出）≠"圆润的脸"（软组织饱满）。
  反例：❌ "身形纤细" → ✅ "身形极度瘦削，肩胛骨在衣料下明显凸起，锁骨凹陷投下深阴影，手腕骨节突出如枯枝，手指细长"
...
- **禁止抽象体型词**：不要只写"纤细"、"圆润"、"健壮"作为体型描述——这些是审美判断，不是视觉指令。T2I无法稳定渲染"纤细"，必须用具体的骨骼/脂肪标志物替代：
  反例：❌ "身形纤细" → ✅ "身形极度瘦削，肩胛骨在衣料下凸起，锁骨凹陷投下阴影，手腕骨节突出"
...
═══ t2iStructure 结构化字段书写规则（T2I专用，必须英文，7个字段缺一不可）═══

t2iStructure 是直接传给 Qwen Image 2512 生成角色设定图的prompt。模型在结构化标签数据上训练，
精度比散文高约30%。7个字段使用英文（Qwen 训练数据以英文标签为主）。

【age】年龄锚定（Per-EP变体核心——同一角色不同EP年龄不同）:
  - 少年/青年: 显式年龄范围 + 发育特征（如 "15-year-old teenager, still growing, slight frame"）
  - 中年: 年龄范围 + 初老信号（如 "45-year-old, early grey at temples, deepened nasolabial folds"）
  - 老年: 年龄范围 + 衰老信号（如 "71-year-old, frail, deeply wrinkled, hunched, sparse white hair"）
  - 不变角色: 写表观年龄 + "physically unchanged"（如 "appears 30, physically unchanged"）

【subject】主体：性别、身高（cm）、体型关键词。格式: "性别, 身高cm, 体型描述词"
  例: "male, 162cm, thin, slightly hunched, slow gait"

【body】体态细节：骨骼标志物（肩胛骨/锁骨/肋骨/手腕/指关节）、体脂状态、肌肉状态、姿势特征
  例: "narrow shoulders, loose skin at neck, prominent hand veins, bowed back, thin wrists"

【face】面部：脸型骨架+五官+皮肤质感+表情。区分骨架和软组织，如 "round bone structure, gaunt from emaciation, sunken cheeks"
  例: "deep forehead wrinkles, crow's feet, sunken cheeks, age spots, thin white eyebrows"

【hair】发型：颜色、长度、质地、样式
  例: "sparse white hair, receding hairline, thin wispy beard"

【clothing】服装：款式+合身度+材质+颜色。关键：衣服与身体的关系（松垮/紧绷/合身）
  例: "faded yellow dragon robe with gold embroidery, slightly too large on aged frame, jade belt, fabric sags at shoulders"

【lighting】光影：光位+质感+对体型的效果
  例: "warm front light, soft shadows, emphasizes skin texture and age marks"

每个字段1-2句英文，禁止散文长段落。不要包含色彩调色板（已在 description 中）。
